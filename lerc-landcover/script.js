...
const Lerc8bitColorLayer = L.GridLayer.extend({
  ...
  draw: function (tile) {
    ...
    const rasterAttributeTableFeatures =
      this.options.rasterAttributeTable.features;
    ...
    for (let i = 0; i < width * height; i++) {
      ...
      const attributes = rasterAttributeTableFeatures.find(
        (info) => info.attributes.Value === pixelValue
      ).attributes;
...
fetch(`${landcoverImageServerUrl}/rasterattributetable?f=json`, {
  method: "GET",
})
